...
using JameNegar.Forms.JameNegarManager.DocumentManager.View;
using JameNegar.Interfaces;
using JameNegar.Models;
using JameNegar.Templates;
using static JameNegar.DedicatedFunctions;
using static JameNegar.Models.TemplateRelationshipModel;
    ...
    public partial class CreateDocumentSlide6 : UserControl, IChangeTransitionDocumentManager, ICloseForm
    {
        ...
        private void createNewDocument()
        {
            ...
            if (threadCreateDocument.ThreadState != System.Threading.ThreadState.Aborted && threadCreateDocument.ThreadState != threadNotStartAndAbort)
            {
                ...
                {
                    string shivanegarTemplatesPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "Microsoft", "Templates", "JameNegarTemplates");
            ...
            return new Dictionary<string, string>()
                {
                    ...
                    { ContentControlNames._field_Location_Fa.ToString(),locationFa},
        ...
        private void create()
        {
            ...
            DedicatedFunctions.addVariable(specifiedDocument, VariableFieldIDs._variable_field_Location_Fa.ToString(), locationFa);
            ...
            try
            {
                ...
                // استفاده از نام متغیر جدید برای جلوگیری از تداخل
                var contentsDict = getValueList(university, (TemplateTypes)templateType, (DocumentTypes)documentType);
                string[] ccNames = Enum.GetNames(typeof(ContentControlNames));
                ...
                foreach (string ccName in ccNames)
                {
                    try
                    {
                        ...
                        DedicatedFunctions.getLockStatusContentControl(ccName, out bool lockContentControl, out bool lockContent);

                        string content;
                        if (!contentsDict.TryGetValue(ccName, out content))
                        {
                            // اگر در دیکشنری نبود، فقط قفل‌ها را تنظیم کن
                            ContentControls ccs2 = specifiedDocument.SelectContentControlsByTag(ccName);
                            if (ccs2 != null)
                            {
                        ...
                        string title = DedicatedFunctions.getContentControlTitle(ccName);

                        // منطق mustBeEmpty
                        if (ccName == ContentControlNames._field_Advisor_Title_Fa.ToString() || ccName == ContentControlNames._field_Advisor_Fa.ToString())
                        {
                        ...
                        else if (ccName == ContentControlNames._field_Advisor_Title_En.ToString() || ccName == ContentControlNames._field_Advisor_En.ToString())
                        {
                        ...
                        else if (ccName == ContentControlNames._field_AreaOfStudy_Title_Fa.ToString() || ccName == ContentControlNames._field_AreaOfStudy_Fa.ToString())
                        {
                        ...
                        else if (ccName == ContentControlNames._field_AreaOfStudy_Title_En.ToString() || ccName == ContentControlNames._field_AreaOfStudy_En.ToString())
                        {
                        ...
                        ContentControls ccs = specifiedDocument.SelectContentControlsByTag(ccName);
                        if (ccs != null)
                        {
                            foreach (Microsoft.Office.Interop.Word.ContentControl cc in ccs)
                            {
                                ...
                                {
                                    customActionInUniversity(specifiedDocument, ccName);
                                }
                            }
                        }
                    }
                    ...
                    {
                        System.Windows.MessageBox.Show($"Error in ContentControl: {ccName}\n{ex.Message}");
                        // ادامه بده، کل سند را متوقف نکن
                    }
                }
            ...
            {
                System.Windows.MessageBox.Show("Error in final Configuration: " + ex.Message + "\n\n" + ex.StackTrace);
                cancelCreateDocument();
                return;
            }
            #endregion
